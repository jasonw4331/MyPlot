...
class EventListener implements Listener
{
	...
	public function onPlayerMove(PlayerMoveEvent $event) {
		...
		if ($plot !== null and $plot !== $plotFrom) {
			$this->plugin->getServer()->getPluginManager()->callEvent($ev = new MyPlotPlayerEnterPlotEvent($this->plugin, "MyPlot", $plot, $event->getPlayer()));
			if($ev->isCancelled()) {
				$event->setCancelled();
				return;
			}
		...
		if ($plotFrom !== null and ($plot === null or $plot !== $plotFrom)) {
			$this->plugin->getServer()->getPluginManager()->callEvent($ev = new MyPlotPlayerLeavePlotEvent($this->plugin, "MyPlot", $plotFrom, $event->getPlayer()));
		}
	}
}